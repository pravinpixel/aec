...
@section('admin-content')
                            ...
                            <div class="page-title-right mt-0">
                                <ol class="breadcrumb align-items-center m-0">
                                    ...
                                    <li class="breadcrumb- ms-2">
                                        <i type="button" onclick="goBack()"
                                            class="mdi mdi-backspace text-danger fa-2x"></i>
                                    </li>
                                </ol>
                            </div>
                            <h4 class="page-title">
    ...
    <div class="modal fade" id="proposalModal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel"
        aria-hidden="true">
    ...
    <script>
        ...
        app.controller('WizzardCtrl', function($scope, $http, API_URL, $location) {
            ...
            $scope.GetCommentsData = function() {
                $http.get(API_URL + 'admin/api/v2/customers-enquiry/' + {{ $data->id ?? ' ' }}).then(function(
                    res) {
                    ...
                    $scope.htmlEditorOptions = {
                        height: 300,
                        value: (res.data.additional_infos == null) ? '' : res.data.additional_infos
                            .comments,
                        mediaResizing: {
                            enabled: false,
                        },
            ...
            $scope.showCommentsToggle = function(modalstate, type, header) {
                ...
                switch (modalstate) {
                    case 'viewConversations':
                        $http.get(API_URL + 'admin/show-comments/' + $scope.enquiry_id + '/type/' + type).then(
                            function(response) {
            ...
            $scope.getDocumentView = (file) => {
                $http({
                    ...
                    if (file.file_type == 'pdf')
                        var htmlPop = '<iframe id="iframe" src="data:application/pdf;base64,' + res
                            .data +
                            '"  width="100%" height="1000" allowfullscreen webkitallowfullscreen disableprint=true; ></iframe>';
                    else
                        var htmlPop = '<embed width="100%" height="1000" src="data:image/png;base64,' +
                            res.data + '"></embed>';
        ...
        app.controller('Tech_Estimate', function($scope, $http, API_URL, $location) {
            ...
            $http.get(API_URL + 'admin/api/v2/customers-technical-estimate/' + {{ $data->id ?? ' ' }}).then(
                function(response) {
            ...
            $scope.Add_building = function() {
                ...
                $http.get(API_URL + 'admin/api/v2/customers-technical-estimate/' + {{ $data->id ?? ' ' }})
                    .then(function(response) {
            ...
            $scope.updateTechnicalEstimate = function() {
                ...
                validator.forEach(element => {
                    ...
                    for (let i = 0; i < element.building_component_number.length; i++) {
                        if (element.building_component_number[i].name == null || element
                            .building_component_number[i].name == '') {
                            validatorName = true;
                        }
                        if (element.building_component_number[i].sqfeet == null || element
                            .building_component_number[i].sqfeet == '') {
                ...
                if ($scope.building_building.length == 0 || $scope.building_building[0]
                    .building_component_number.length == 0 || $scope.building_building == []) {
                    Message('danger', "You Can't Update Empty Building !");
                    $http.get(API_URL + 'admin/api/v2/customers-technical-estimate/' + {{ $data->id ?? ' ' }})
                        .then(function(response) {
                            $scope.enquiry = response.data;
                ...
                $http({
                    method: "POST",
                    url: API_URL + 'admin/api/v2/customers-technical-estimate/' +
                        {{ $data->id ?? ' ' }},
                    data: {
                        data: $scope.building_building,
                        history: true,
                        html: $("#root_technical_estimate").html()
                    },
                }).then(function successCallback(response) {
                    $scope.getWizradStatus();
                    Message('success', response.data.msg);
            ...
            $scope.assignTechnicalEstimate = (user, technical_estimate_assign_for) => {
                ...
                $http.post(`${API_URL}technical-estimate/assign-user/${enquiryId}`, {
                        assign_to: assign_to,
                        type: technical_estimate_assign_for
                    })
                    .then(function successCallback(res) {
                        if (res.data.status) {
            ...
            $scope.sendAssignTechInboxComments = function(type, chatSection) {
                ...
                $http({
                    ...
                    $scope.showTechCommentsToggle('viewAssingTechicalConversations',
                        'techical_estimation', chatSection);
            ...
            $scope.showTechCommentsToggle = function(modalstate, type, docId) {
                ...
                switch (modalstate) {
                    ...
                    case 'viewTechicalDocsConversations':
                        $http.get(API_URL + 'admin/show-tech-comments/' + $scope.enquiry_id + '/type/' + $scope
                            .technical_docType_id).then(function(response) {
                            $scope.TechcommentsData = response.data.chatHistory;
                            $scope.chatType = response.data.chatType;
                        ...
                        break;
                    case 'viewAssingTechicalConversations':
                        $http.get(API_URL + 'admin/show-comments/' + $scope.enquiry_id + '/type/' + $scope
                            .technical_docType_id).then(function(response) {
                            $scope.TechcommentsData = response.data.chatHistory;
                            $scope.chatType = response.data.chatType;
                            $('#assing-viewConversations-modal').modal('show');
            ...
            $scope.sendTechInboxComments = function(type, chatSection) {
                ...
                $http({
                    ...
                    document.getElementById("Inbox__commentsForm").reset();
                    $scope.showTechCommentsToggle('viewTechicalDocsConversations',
                        'techical_estimation', $scope.technical_docType_id);
                    Message('success', response.data.msg);
                }, function errorCallback(response) {
                    Message('danger', response.data.errors);
                });
        ...
        app.controller('Cost_Estimate', function($scope, $http, $timeout, API_URL) {
            ...
            $scope.addWoodTemplate = (type) => {
                ...
                $http.post(`${API_URL}admin/cost-estimate-template`, {
                        data: templateData
                    })
                    .then(function successCallback(res) {
                        if (res.data.status) {
            ...
            $scope.assignUserToCostestimate = (user, cost_estimate_assign_for) => {
                ...
                $http.post(`${API_URL}cost-estimate/assign-user/${enquiryId}`, {
                        assign_to: assign_to,
                        type: cost_estimate_assign_for
                    })
                    .then(function successCallback(res) {
            ...
            $scope.getWizradStatus = function() {
                $http.get(API_URL + 'admin/api/v2/customers-enquiry/' + {{ $data->id ?? ' ' }}).then(function(
                    ...
                    $scope.ResultPrecastComponent = JSON.parse(res.data.cost_estimate
                        .precast_build_json);
                    $scope.EngineeringEstimate = $scope.ResultEngineeringEstimate.costEstimate;
                    $scope.ResetCloneEngineeringEstimate = JSON.parse(res.data.cost_estimate
                        .build_json);
            ...
            $scope.UpdateCostEstimate = function(type) {
                ...
                let total = $scope.ResultEngineeringEstimate.total.totalSum + $scope.ResultPrecastComponent
                    .total.totalSum;
                $http({
                    method: "POST",
            ...
            $scope.sendAssignCostEstiComments = function(type, chatSection) {
                ...
                $http({
                    ...
                    document.getElementById("Inbox__commentsForm").reset();
                    $scope.showTechCommentsToggle('viewConversations', 'cost_estimation_assign',
                        chatSection);
                    Message('success', response.data.msg);
                }, function errorCallback(response) {
                    Message('danger', response.data.errors);
            ...
            $scope.showTechCommentsToggle = function(modalstate, type, docId) {
                ...
                $scope.module = null;
                $scope.technical_docType_id = docId;
                $http.get(API_URL + 'admin/show-comments/' + $scope.enquiry_id + '/type/' + type).then(function(
                    response) {
                    $scope.commentsData = response.data.chatHistory;
                    $scope.chatType = response.data.chatType;
                    $('#assing-viewConversations-modal').modal('show');
            ...
            $scope.addDynamicColumn = (index, columnName) => {
                ...
                $http.post(`${API_URL}wood-estimate`, {
                    name: columnName
                }).then((res) => {
                    console.log(res.data);
                })
            ...
            $scope.addComponent = function(index) {
                let componentFirstObj = JSON.parse(JSON.stringify($scope.EngineeringEstimate[index].Components[
                    0]));
                ...
                let newObj = {
                    ...$scope.NewCostEstimate.Components[0],
                    ...{
                        Dynamics: removeVal
                    }
                }
        ...
        //         ${API_URL}admin/calculate-cost-estimate/delete/${id}/${type}`,
            ...
            $scope.savePrecastComponent = () => {
                $http.post(`${API_URL}precast-estimate`, {
                        name: $scope.precast_component_name,
                        hours: $scope.precast_component_hours
                    })
                    .then(function successCallback(response) {
                        Message('success', response.data.msg);
            ...
            return {
                ...
                link: function(scope, element, attrs) {
                    ...
                    //         let $TotalSum       = 0;
                    //         let $TotalRibSum    = 0;
                    //         console.log(scope.CostEstimate.ComponentsTotals.Dynamics)
                    //         return 0;
                    //         scope.CostEstimate.ComponentsTotals.Dynamics.forEach( (item, index) => {

                    //             // designScope percentage validation
                    //             if(scope.CostEstimate.Components[index].DesignScope > 100) {
                    //                 scope.CostEstimate.Components[index].DesignScope = 100;
                    //             } else if(scope.CostEstimate.Components[index].DesignScope < 0) {
                    //                 scope.CostEstimate.Components[index].DesignScope = 1;
                    //             }

                    //             // complexity validation
                    //             if(scope.CostEstimate.Components[index].Complexity > 2) {
                    //                 scope.CostEstimate.Components[index].Complexity = 2;
                    //             } else if(scope.CostEstimate.Components[index].Complexity < 1) {
                    //                 scope.CostEstimate.Components[index].Complexity = 1;
                    //             }

                    //             scope.CostEstimate.Components[index].Dynamics[index].Sum  = getNum(((scope.CostEstimate.Components[index].Sqm * scope.CostEstimate.Components[index].Complexity * scope.CostEstimate.Components[index].Dynamics[index].PriceM2  ) * scope.CostEstimate.Components[index].DesignScope) / 100);
                    //             $TotalPriceM2   += Number(scope.CostEstimate.Components[index].Dynamics[index].PriceM2);
                    //             $TotalSum       += Number(scope.CostEstimate.Components[index].Dynamics[index].Sum);
                    //         });
                    //         console.log(' scope.CostEstimate.Components[index].Rib.Sum', scope.CostEstimate.Components[index].Rib.Sum)
                    //         if(scope.CostEstimate.Components[index].Rib.Sum != 0 && scope.CostEstimate.Components[index].Rib.Sum != '' && scope.CostEstimate.Components[index].Rib.Sum != 'undefined'){
                    //             scope.CostEstimate.Components[index].Sqm = 0;
                    //             $TotalRibSum = scope.CostEstimate.Components[index].Rib.Sum * scope.CostEstimate.Components[index].TotalCost.PriceM2 ;
                    //             scope.CostEstimate.Components[index].TotalCost.Sum = getNum($TotalRibSum);
                    //         } else {
                    //             scope.CostEstimate.Components[index].TotalCost.Sum = getNum($TotalSum);
                    //             scope.CostEstimate.Components[index].TotalCost.PriceM2 = getNum($TotalPriceM2);
                    //         }
                    ...
                    element.on('keyup', function() {
                        ...
                        scope.CostEstimate.ComponentsTotals.Dynamics.forEach((item, index) => {
                            ...
                            // designScope percentage validation
                            if (scope.CostEstimate.Components[scope.index].DesignScope >
                                100) {
                                scope.CostEstimate.Components[scope.index].DesignScope =
                                    100;
                            } else if (scope.CostEstimate.Components[scope.index]
                                .DesignScope < 0) {
                            ...
                            scope.CostEstimate.Components[scope.index].Dynamics[index].Sum =
                                getNum(((scope.CostEstimate.Components[scope.index].Sqm *
                                        scope.CostEstimate.Components[scope.index]
                                        .Complexity * scope.CostEstimate.Components[
                                            scope.index].Dynamics[index].PriceM2) *
                                    scope.CostEstimate.Components[scope.index]
                                    .DesignScope) / 100);
                            $TotalPriceM2 += Number(scope.CostEstimate.Components[scope
                                .index].Dynamics[index].PriceM2);
                            $TotalSum += Number(scope.CostEstimate.Components[scope.index]
                                .Dynamics[index].Sum);
                        });
                        console.log(' scope.CostEstimate.Components[scope.index].Rib.Sum', scope
                            .CostEstimate.Components[scope.index].Rib.Sum)
                        if (scope.CostEstimate.Components[scope.index].Rib.Sum != 0 && scope
                            .CostEstimate.Components[scope.index].Rib.Sum != '' && scope
                            .CostEstimate.Components[scope.index].Rib.Sum != 'undefined') {
                            scope.CostEstimate.Components[scope.index].Sqm = 0;
                            $TotalRibSum = scope.CostEstimate.Components[scope.index].Rib.Sum *
                                scope.CostEstimate.Components[scope.index].TotalCost.PriceM2;
                            scope.CostEstimate.Components[scope.index].TotalCost.Sum = getNum(
                                $TotalRibSum);
                        } else {
                            scope.CostEstimate.Components[scope.index].TotalCost.Sum = getNum(
                                $TotalSum);
                            scope.CostEstimate.Components[scope.index].TotalCost.PriceM2 = getNum(
                                $TotalPriceM2);
                        }
                        ...
                        scope.EngineeringEstimate.forEach((Estimates, estimateIndex) => {
                            ...
                            Estimates.Components.forEach((Component, componentIndex) => {
                                ...
                                if (Component.Rib.Sum != 0 && Component.Rib.Sum !=
                                    '') {
                                    $totalSum += Number(Component.Rib.Sum *
                                        Component.TotalCost.PriceM2);
                                    $totalEstimateSum += Number(Component.Rib.Sum *
                                        Component.TotalCost.PriceM2);
                                } else {
                                    Component.Dynamics.forEach((Dynamic,
                                        dynamicIndex) => {
                                        Estimates.ComponentsTotals.Dynamics[
                                            dynamicIndex].Sum += Number(
                                            Dynamic.Sum);
                                        Estimates.ComponentsTotals.Dynamics[
                                                dynamicIndex].PriceM2 =
                                            getNum(Estimates
                                                .ComponentsTotals.Dynamics[
                                                    dynamicIndex].Sum /
                                                $totalSql_);
                                        $totalPrice += Number(Dynamic
                                            .PriceM2);
                                        $totalSum += Number(Dynamic.Sum);
                                        $totalEstimateSum += Number(Dynamic
                                            .Sum);
                                        Estimates.ComponentsTotals.Dynamics[
                                            dynamicIndex].Sum = getNum(
                                            Estimates.ComponentsTotals
                                            .Dynamics[dynamicIndex].Sum);
                                    });
                                }

                            });
                            Estimates.ComponentsTotals.TotalCost.Sum = getNum($totalSum);
                            Estimates.ComponentsTotals.TotalCost.PriceM2 = getNum(
                                $totalSum / $sqmTotal);
                            Estimates.ComponentsTotals.Sqm = getNum($sqmTotal);
                            Estimates.ComponentsTotals.Rib.Sum = getNum($ribTotal);

                        });
                        scope.ResultEngineeringEstimate.total.totalArea = getNum(
                            $totalEstimateArea);
                        scope.ResultEngineeringEstimate.total.totalSum = getNum($totalEstimateSum);
                        scope.ResultEngineeringEstimate.total.totalPris = getNum($totalEstimateSum /
                            $totalEstimateArea);
                        scope.ResultEngineeringEstimate.costEstimate = scope.EngineeringEstimate;
                        scope.$apply();
                    });
                ...
                link: function(scope, element, attrs) {
                    element.on('change', function() {
                        ...
                        $http({
                            ...
                            params: {
                                component_id: scope.C.building_component_id,
                                type_id: scope.C.type_id
                            }
                        }).then(function success(res) {
                            response = res.data;
                            scope.EngineeringEstimate.forEach((Estimates,
                                estimateIndex) => {
                                if (scope.$parent.firstIndex == estimateIndex) {
                                    Estimates.Components.forEach((Component,
                                        componentIndex) => {
                                        if (scope.index == componentIndex) {
                                            Component.Dynamics.forEach((
                                                Dynamic,
                                                dynamicIndex) => {
                                                if (Estimates
                                                    .Components[
                            ...
                            let finalJson = {
                                ...scope.ResultEngineeringEstimate.costEstimate,
                                ...scope.EngineeringEstimate
                            };
                            scope.ResultEngineeringEstimate.costEstimate = JSON.parse(JSON
                                .stringify(finalJson));
                            ...
                            setTimeout(() => {
                                ...
                                let $TotalSum = 0;
                                let $TotalRibSum = 0;
                                scope.CostEstimate.ComponentsTotals.Dynamics
                                    .forEach((item, index) => {

                                        // designScope percentage validation
                                        if (scope.CostEstimate.Components[scope
                                                .index].DesignScope > 100) {
                                            scope.CostEstimate.Components[scope
                                                .index].DesignScope = 100;
                                        } else if (scope.CostEstimate
                                            .Components[scope.index]
                                            .DesignScope < 0) {
                                            scope.CostEstimate.Components[scope
                                                .index].DesignScope = 1;
                                        }
                                        ...
                                        scope.CostEstimate.Components[scope
                                                .index].Dynamics[index].Sum =
                                            getNum(((scope.CostEstimate
                                                    .Components[scope.index]
                                                    .Sqm * scope
                                                    .CostEstimate
                                                    .Components[scope.index]
                                                    ...
                                                    .CostEstimate
                                                    .Components[scope.index]
                                                    .Dynamics[index].PriceM2
                                                ) * scope.CostEstimate
                                                .Components[scope.index]
                                                .DesignScope) / 100);
                                        $TotalPriceM2 += Number(scope
                                            .CostEstimate.Components[scope
                                                .index].Dynamics[index]
                                            .PriceM2);
                                        $TotalSum += Number(scope.CostEstimate
                                            .Components[scope.index]
                                            .Dynamics[index].Sum);
                                    });
                                console.log(
                                    ' scope.CostEstimate.Components[scope.index].Rib.Sum',
                                    scope.CostEstimate.Components[scope.index]
                                    .Rib.Sum)
                                if (scope.CostEstimate.Components[scope.index].Rib
                                    .Sum != 0 && scope.CostEstimate.Components[scope
                                        .index].Rib.Sum != '' && scope.CostEstimate
                                    .Components[scope.index].Rib.Sum != 'undefined'
                                ) {
                                    scope.CostEstimate.Components[scope.index].Sqm =
                                        0;
                                    $TotalRibSum = scope.CostEstimate.Components[
                                            scope.index].Rib.Sum * scope
                                        .CostEstimate.Components[scope.index]
                                        .TotalCost.PriceM2;
                                    scope.CostEstimate.Components[scope.index]
                                        .TotalCost.Sum = getNum($TotalRibSum);
                                } else {
                                    scope.CostEstimate.Components[scope.index]
                                        .TotalCost.Sum = getNum($TotalSum);
                                    scope.CostEstimate.Components[scope.index]
                                        .TotalCost.PriceM2 = getNum($TotalPriceM2);
                                }
                                ...
                                scope.EngineeringEstimate.forEach((Estimates,
                                    ...
                                    Estimates.Components.forEach((Component,
                                        ...
                                        $sqmTotal += Number(
                                            Component.Sqm);
                                        $totalEstimateArea +=
                                            Number(Component.Sqm);
                                        $ribTotal += Number(
                                            Component.Rib.Sum);
                                        if (Component.Rib.Sum !=
                                            0 && Component.Rib
                                            .Sum != '') {
                                            $totalSum += Number(
                                                Component.Rib
                                    ...
                                    Estimates.ComponentsTotals.TotalCost
                                        .Sum = getNum($totalSum);
                                    Estimates.ComponentsTotals.TotalCost
                                        .PriceM2 = getNum($totalSum /
                                            $sqmTotal);
                                    Estimates.ComponentsTotals.Sqm = getNum(
                                        $sqmTotal);
                                    Estimates.ComponentsTotals.Rib.Sum =
                                        getNum($ribTotal);

                                });
                                scope.ResultEngineeringEstimate.total.totalArea =
                                    getNum($totalEstimateArea);
                                scope.ResultEngineeringEstimate.total.totalSum =
                                    getNum($totalEstimateSum);
                                scope.ResultEngineeringEstimate.total.totalPris =
                                    getNum($totalEstimateSum / $totalEstimateArea);
                                scope.ResultEngineeringEstimate.costEstimate = scope
                                    .EngineeringEstimate;
                                scope.$apply();
            ...
            return {
                ...
                link: function(scope, element, attrs) {
                    let eventHandle = () => {
                        ...
                        const precast_component = scope.precastEstimateTypes.find(precastEstimateType =>
                            scope.PrecastEstimate.Components[scope.index].precast_component ==
                            precastEstimateType.id);
                        ...
                        if (scope.PrecastEstimate.Components[scope.index].no_of_staircase != 0) {
                            scope.PrecastEstimate.Components[scope.index].std_work_hours = getNum(scope
                                .PrecastEstimate.Components[scope.index].no_of_staircase *
                                precast_component.hours);
                        } else {
                            scope.PrecastEstimate.Components[scope.index].std_work_hours = getNum(scope
                                .PrecastEstimate.Components[scope.index].no_of_new_component *
                                precast_component.hours);
                        }
                        scope.PrecastEstimate.Components[scope.index].total_work_hours = getNum(Number(
                                scope.PrecastEstimate.Components[scope.index].std_work_hours) +
                            Number(scope.PrecastEstimate.Components[scope.index]
                                .additional_work_hours));
                        ...
                        scope.PrecastEstimate.Components[scope.index].total_engineering_cost = Number(
                                scope.PrecastEstimate.Components[scope.index].engineering_cost) +
                            Number(scope.PrecastEstimate.Components[scope.index].total_central_approval)
                        let $total_sqm = 0;
                        ...
                        scope.PrecastEstimate.total_additional_work_hours =
                            $total_additional_work_hours;
        ...
        app.controller('Proposal_Sharing', function($scope, $http, API_URL, $location, $timeout) {
            ...
            $scope.sendMailToCustomer = function(proposal_id) {
                $http.post(API_URL + 'admin/proposal/enquiry/' + {{ $data->id }} + '/send-mail/' +
                    proposal_id).then(function(response) {
                    Message('success', response.data.msg);
                    // $scope.getWizradStatus();
                    $scope.getProposesalData();
                });
            ...
            $scope.sendMailToCustomerVersion = function(proposal_id, Vid) {
                $http.post(API_URL + 'admin/proposal/enquiry/' + {{ $data->id }} + '/send-mail/' +
                    proposal_id + '/version/' + Vid).then(function(response) {
                    Message('success', response.data.msg);
                    // $scope.getWizradStatus();
                    $scope.getProposesalData();
                    $timeout(function() {
            ...
            $scope.moveToProject = () => {
                $http.post(API_URL + 'proposal-move-to-project/' + {{ $data->id }}).then(function(
                    response) {
                    $timeout(function() {
                        window.onbeforeunload = null;
                    });
            ...
            $scope.ViewEditPropose = function(proposal_id, update_status = true) {
                ...
                $http.get(API_URL + 'admin/proposal/enquiry/' + {{ $data->id }} + '/edit/' + proposal_id)
                    .then(function(response) {
            ...
            $scope.ViewEditProposeVersions = function(proposal_id, Vid, update_status = true) {
                $scope.proposalModal = update_status;
                $http.get(API_URL + 'admin/proposal/enquiry/' + {{ $data->id }} + '/edit/' + proposal_id +
                    '/version/' + Vid).then(function(response) {
                    $scope.edit_proposal = response.data;
            ...
            $scope.ViewPropsalVersions = function(proposal_id) {
                $http.get(API_URL + 'admin/proposal/enquiry/' + {{ $data->id }} + '/versions/' +
                    proposal_id).then(function(response) {
            ...
            $scope.DuplicatePropose = function(proposal_id) {
                $http.put(API_URL + 'admin/proposal/enquiry/' + {{ $data->id }} + '/duplicate/' +
                    proposal_id).then(function(response) {
                    $scope.edit_proposal = response.data;
                    Message('success', response.data.msg);
            ...
            $scope.DuplicateProposalVersion = (proposal_id) => {
                $http.put(API_URL + 'admin/proposal/enquiry/' + {{ $data->id }} + '/duplicate-version/' +
                    proposal_id).then(function(response) {
                    $scope.edit_proposal = response.data;
                    Message('success', response.data.msg);
                    $scope.getProposesalData();
            ...
            $scope.DeletePropose = function(proposal_id) {
                $http.delete(API_URL + 'admin/proposal/enquiry/' + {{ $data->id }} + '/edit/' +
                    proposal_id).then(function(response) {
                    Message('success', response.data.msg);
                    $scope.getProposesalData();
                });
            ...
            $scope.updateProposalMail = function(proposalId, modal) {
                ...
                $http({
                    method: "PUT",
                    url: API_URL + 'admin/proposal/enquiry/' + {{ $data->id }} + '/edit/' +
                        proposalId,
                    data: $.param($scope.sendCommentsData),
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded'
            ...
            $scope.updateProposalVersionMail = function(modal) {
                ...
                $http({
                    method: "PUT",
                    url: API_URL + 'admin/proposal/enquiry/' + {{ $data->id }} + '/edit/' + $scope
                        .proposalVersionId + '/version/' + $scope.proposalVId,
            ...
            $scope.showCommentsToggle = (reference_id, version) => {
                ...
                $http.get(API_URL + 'admin/show-comments/' + $scope.enquiry_id + '/' + $scope.version + '/' +
                    $scope.reference_id).then(function(response) {
                    $scope.commentsData = response.data.chatHistory;
                    $scope.chatType = response.data.chatType;
        ...
        app.controller('Customer_response', function($scope, $http, API_URL, $location, $timeout) {
            ...
            $scope.moveToProject = () => {
                ...
                if (assigned_to == false) {
                    Message('danger', 'Assign field required');
                    return false;
                }
                $http.post(API_URL + 'customer-response/move-to-project', {
            ...
            $scope.manualMoveToProject = () => {
                ...
                if (assigned_to == false) {
                    Message('danger', 'Select field required');
                    return false;
                }
                $http.post(API_URL + 'customer-response/move-to-project', {
            ...
            $scope.assignToProject = () => {
                ...
                $http.post(API_URL + 'customer-response/assign-to-project', {
                    assigned_to: assigned_to,
                    enquiry_id,
                    enquiry_id
                }).then(function successfunction(res) {
            ...
            // followup date
            $scope.updateFollow = () => {
                if ($scope.customer_response_obj.follow_up_date == null || $scope.customer_response_obj
                    .follow_up_date == '') {
