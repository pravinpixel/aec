					...
					<form class="form" id="projectform" enctype="multipart/form-data">
						...
						<div class="d-flex justify-content-between border-top mt-5 pt-10">
							...
							<div>
								...
								<button type="button" class="btn btn-primary font-weight-bolder text-uppercase px-9 py-4" onclick="saveProject();">Save Project</button>
							</div>
						</div>
						<!--end::Wizard Actions-->
					</form>
...
<script>
	...
	function setEnquiryData(response)
		{
			var pd = response.projectdata;
			...
			if (pd.projectname!=null) {enquiry.projectName=pd.projectname;}else {enquiry.projectName="";}
			if (pd.projectcode!=null) {enquiry.projectCode=pd.projectcode;enquiryProjectCode=pd.projectcode;}else {enquiry.projectCode="";}
			
			if (pd.address!=null) {enquiry.siteAddress=pd.address;}else {enquiry.siteAddress="";}
			if (pd.postcode!=null) {enquiry.postCode=pd.postcode;}else {enquiry.postCode="";}
			if (pd.place!=null) {enquiry.place=pd.place;}else {enquiry.place="";}
			if (pd.country!=null) {enquiry.country=pd.country;}else {enquiry.country="";}
			
			if (pd.state!=null) {enquiry.state=pd.state;}else {enquiry.state="";}
			
			if (pd.typeofproject!=null) {enquiry.typeofProject=pd.typeofproject;}else {enquiry.typeofProject="";}
			if (pd.typeofbuilding!=null) {enquiry.typeofBuilding=pd.typeofbuilding;}else {enquiry.typeofBuilding="";}
			if (pd.noofbuildings!=null) {enquiry.numberofBuildings=pd.noofbuildings;}else {enquiry.numberofBuildings="";}
			if (pd.typeofdelivery!=null) {enquiry.typeofDelivery=pd.typeofdelivery;}else {enquiry.typeofDelivery="";}
			
			if (pd.startdate!=null) {enquiry.startDate=pd.startdate;}else {enquiry.startDate="";}
			
			if (pd.deliverydate!=null) {enquiry.deliveryDate=pd.deliverydate;}else {enquiry.deliveryDate="";}
			if (pd.contactname!=null) {enquiry.contactName=pd.contactname;}else {enquiry.contactName="";}
			if (pd.telephone!=null) {enquiry.telephone=pd.telephone;}else {enquiry.telephone="";}
			if (pd.epost!=null) {enquiry.ePost=pd.epost;}else {enquiry.ePost="";}
			
			if (pd.additionalinfo!=null) {enquiry.additionalInfo=pd.additionalinfo;}else {enquiry.additionalInfo="";}
			if (pd.otherservice!=null) {enquiry.otherservice=pd.otherservice;}else {enquiry.otherservice="";}
			
			if (pd.userid!=null) {enquiryUserId=pd.userid;}else {enquiryUserId=0;}
			
			
			$("#name").val(enquiry.projectName);
			$("#address_line_1").val(enquiry.siteAddress);
			$("#postal_code").val(enquiry.postCode);
			$("#city").val(enquiry.place);
			
			
			$("#project_type").val(enquiry.typeofProject);
			$("#construction_type").val(enquiry.typeofBuilding);
			$("#start_date").val(enquiry.startDate);
			$("#end_date").val(enquiry.deliveryDate);
			
			$("#country").val(enquiry.country);
			loadStatesSelectItems();
			$("#state_or_province").val(enquiry.state);
	...
	function getCountryId(countryname) {
		for(var i=0;i<countrywisestatelist.length;i++)
		{
			if (countrywisestatelist[i].country==countryname)
			{
				return(i);
			}
		}
	...
	$(document).ready(function() {
		...
		enquiryid = getUrlParameter('enquiryid');
