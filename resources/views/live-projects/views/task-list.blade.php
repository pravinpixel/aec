        ...
        document.addEventListener("DOMContentLoaded", function() {
                    ...
                    if (response.data.status) {
                        ...
                        $(".sortable").sortable({
                            ...
                            stop: function(event, ui) {
                                ...
                                axios.post(
                                    `{{ route('live-project.sub-task-change-order') }}`, {
                                        data: data
                                    }).then((response) => {
            ...
            deleteLiveProjectSubSubTask = (sub_sub_task_id, element) => {
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete(
                                `{{ route('live-project.sub-sub-task.delete') }}/${sub_sub_task_id}`)
                            .then((response) => {
                                if (response.data.status) {
                                    element.parentNode.parentNode.remove()
                                    $(`#${element.attributes[1]['nodeValue']}`).html(response.data
                                        .progress)
            ...
            deleteLiveProjectSubTask = (sub_task_id, element) => {
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete(`{{ route('live-project.sub-task.delete') }}/${sub_task_id}`).then(
                            (response) => {
                                if (response.data.status) {
                                    element.parentNode.parentNode.remove()
                                    Alert.success('Successfully Deleted !')
            ...
            storeSubTask = () => {
                ...
                if (errorCounts === 0) {
                    ...
                    axios.post(`{{ route('live-project.sub-task.create') }}/${sub_task_id}`, InputData).then((
                        response) => {
                        if (response.data.status) {
            ...
            setTaskStatus = (sub_task_id, task_id, element) => {
                var inputs = $(
                    `.${element.parentNode.parentNode.parentNode.classList[0]} input , .${element.parentNode.parentNode.parentNode.classList[0]} select`
                    )
                var errorCounts = 0
            ...
            deleteSubtask = (id) => {
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.isConfirmed) {
                        axios.delete(`{{ route('live-project.task-list-delete') }}/${id}`).then(() => {
                            Alert.success('Successfully Deleted !')
                            setAllTask()
                        })
