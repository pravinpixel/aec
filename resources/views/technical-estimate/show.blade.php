                ...
                <div class="container-fluid d-none" id="TechEstimateController">
                    <div class="card shadow-sm border">
                        <div class="card-header">
                            <ul class="nav nav-pills bg-nav-pills nav-justified m-0 border-bottom">
                                <li class="nav-item">
                                    <a href="#project_summary" data-bs-toggle="tab" aria-expanded="false"
                                        class="nav-link d-flex justify-content-center align-items-center rounded-0">
                                        <div class="inner-circle bg-secondary">
                                            <img src="{{ asset('public/assets/icons/information.png') }}"
                                                class="w-50 invert">
                                        </div>
                                        <span class="ps-3">Project Summary</span>
                                    </a>
                                </li>
                                <li class="nav-item">
                                    <a href="#technical_estimate" data-bs-toggle="tab" aria-expanded="true"
                                        class="nav-link d-flex justify-content-center align-items-center rounded-0 active">
                                        <div class="inner-circle bg-secondary">
                                            <img src="{{ asset('public/assets/icons/budget.png') }}" class="w-50 invert">
                                        </div>
                                        <span class="ps-3">Technical Estimate</span>
                                    </a>
                                </li>
                            </ul>
                        </div>
                        <div class="card-body">
                            <div class="tab-content">
                                ...
                                <div class="tab-pane show active" id="technical_estimate"
                                    ng-controller="TechEstimateController">
    ...
    <script>
        ...
        app.controller('ProjectSummaryController', function($scope, $http, API_URL, $location) {
            ...
            $scope.GetCommentsData = function() {
                $http.get(API_URL + 'admin/api/v2/customers-enquiry/' + {{ $enquiry_id ?? ' ' }}).then(function(
                    res) {
        ...
        app.controller('TechEstimateController', function($scope, $http, API_URL, $location) {
            ...
            $http.get(API_URL + 'admin/api/v2/customers-technical-estimate/' + {{ $enquiry_id ?? ' ' }}).then(
                function(response) {
            ...
            $scope.getWizradStatus = function() {
                $http.get(API_URL + 'admin/api/v2/customers-enquiry/' + {{ $enquiry_id ?? ' ' }}).then(
                    function(res) {
                        $scope.project_summary_status = res.data.progress.status;
                        $scope.technical_estimation_status = res.data.progress.technical_estimation_status;
                        $scope.cost_estimation_status = res.data.progress.cost_estimation_status;
            ...
            $scope.Add_building = function() {
                ...
                $http.get(API_URL + 'admin/api/v2/customers-technical-estimate/' + {{ $enquiry_id ?? ' ' }})
            ...
            $scope.updateTechnicalEstimate = function() {
                ...
                if (BuildingValidator) {
                    ...
                    $http.get(API_URL + 'admin/api/v2/customers-technical-estimate/' +
                        {{ $enquiry_id ?? ' ' }}).then(function(response) {
                        $scope.enquiry = response.data;
                        $scope.enquiry_id = response.data.enquiry_id;
                        $scope.building_building = response.data.building_component;
                    });
