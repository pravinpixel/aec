    ...
    <tbody class="panel"> 
        <tr ng-repeat="viewList in viewLists">
            ...
            <td class="ext-start"  ng-show="viewList.pivot.file_type != 'link'">
                ...
                <a  ng-show="!fileType.includes(viewList.pivot.file_type)" href="javascript:void(0)" ng-click="getDocumentView(viewList) "data-url="{{ url('/') }}/get-enquiry-document/@{{ viewList.pivot.id }}"><i class="document-modal fa fa-eye btn-sm rounded-pill btn btn-outline-info"></i></a>
                <a href="" custom-modal="modal" modal-title="Delete" modal-body="Are you sure you want to perform this action" modal-route="{{ route('customers.enquiry-document') }}" modal-enquiry-id="@{{  viewList.pivot.enquiry_id }}"   modal-view-type="@{{  viewList.slug }}"  modal-id="@{{  viewList.pivot.id }}"  modal-method="DELETE">  
