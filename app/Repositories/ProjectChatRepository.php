...
class ProjectChatRepository implements ProjectChatRepositoryInterface{
    protected $model;

    public function __construct(ProjectChats $projectComment)
    {
        $this->model = $projectComment;
    }
    ...
    public function show(Request $r, $id, $type)
     {
         $result["chatHistory"] = $this->model->where(["project_id"=> $id, "type"=>  $type])->oldest()->get();
    ...
    // }
     ...
     public function getActiveCommentsCountByType($id)
     {
         ...
         return  $this->model->select("type", DB::raw("count(*) as comments_count"))
                                 ->where(['project_id' => $id, 'status' => 0, 'created_by' => $created_by])
                                 ->groupBy('type')
                                 ->get();
     }
